...
from lattice.elastic.rendezvous import (
...
from lattice.elastic.rendezvous.k8s_constants import (
    LATTICE_GROUP,
    LATTICE_VERSION,
    LATTICE_NAMESPACE,
    LATTICE_JOB_PLURAL
...
class EtcdRendezvousHandler(RendezvousHandler):
    ...
    def num_nodes_waiting(self):
        try:
            ...
            # If status is joinable then a new rendezvous has started
            # Force restart in this case
...
class EtcdRendezvous(object):
    ...
    def __init__(
        ...
        try:
            ...
            self.crd_client = k8s_client.CustomObjectsApi()
        except ConfigException:
            self.k8s_client = None

        self._expected_nodes = self._get_expected_nodes()
    ...
    def _query_expected_nodes(self):
        hname = os.getenv('HOSTNAME')
        response = self.k8s_client.read_namespaced_pod(hname, LATTICE_NAMESPACE)
        job_name = response.to_dict()['metadata']['labels']['job-name']
        response = self.crd_client.get_namespaced_custom_object_status(LATTICE_GROUP,
                                                                       LATTICE_VERSION,
                                                                       LATTICE_NAMESPACE,
                                                                       LATTICE_JOB_PLURAL,
                                                                       job_name)
        return response['status']['currentSize']

    def _get_expected_nodes(self):
        if not self.k8s_client:
            return self._num_max_workers

        try:
            num_nodes = self._query_expected_nodes()
            return num_nodes
        except Exception:
            return self._num_max_workers
    ...
    def join_rendezvous(self, expected_version):
        ...
        while True:
            ...
            if len(state["participants"]) >= self._expected_nodes:
                state["status"] = "frozen"
                state["keep_alives"] = []
        ...
        def lease_worker(client, path, ttl, stop_event):
            while True:
                try:
                    ...
                    break
                # TODO: Figure out why there can be raft bug
    ...
    def setup_global_kv_store(self):
        global_store_path = self.get_path(f"/rdzv/{self._run_id}")
        self.create_path_if_not_exists(global_store_path)
        return EtcdStore(etcd_client=self.client, etcd_store_prefix=global_store_path)
...
def _create_etcd_client(params: RendezvousParameters) -> etcd.Client:
    ...
    """
    hostname, port = params.endpoint, int(params.port)
