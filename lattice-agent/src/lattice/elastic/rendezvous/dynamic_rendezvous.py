...
from lattice.elastic.events import (
    NodeState,
    construct_and_record_rdzv_event,
)
from lattice.elastic.rendezvous.etcd_store import EtcdStore
...
# TODO (LATTICE): Provide implementations of both store classes
class PrefixStore:
    def __init__(self):
        pass
...
class DynamicRendezvousHandler(RendezvousHandler):
    ...
    def _get_store(self) -> Store:
        key_prefix = f"lattice.rendezvous.{self._settings.run_id}.{self._state_holder.state.round}"

        return EtcdStore(self._store.client, key_prefix)
