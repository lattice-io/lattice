...
class EtcdStore(Store):
    ...
    def __init__(
        ...
        etcd_client,
        etcd_store_prefix,
        timeout: Optional[datetime.timedelta] = datetime.timedelta(seconds=30)
    ):
    ...
    def compare_set(self, key: str, expected_value: str, desired_value: str) -> bytes:
        """
        Write a value to the key, only if the current value matches `expected_value`

        Returns the new value if it is set or the previous value if it is not
        """
        with etcd.Lock(self.client, 'compare_set'):
            try:
                val = self.client.write(
                    key=self.prefix + self._encode(key),
                    value=self._encode(desired_value),
                    prevValue=self._encode(expected_value)).value
                val = self._decode(val)
            except etcd.EtcdCompareFailed as exc:
                # Can extract the current value from the Exception without having
                # to do another round trip
                # All values on EtcdStore are strings so this is feasible
                cause_str = exc.payload['cause']

                old_val = cause_str.split()[-1][:-1]
                val = self._decode(old_val)
            except etcd.EtcdKeyNotFound:
                self.set(key, desired_value)
                val = desired_value.encode()
    ...
    #
    def _encode(self, value) -> Optional[str]:
        if value == '':
            return None
