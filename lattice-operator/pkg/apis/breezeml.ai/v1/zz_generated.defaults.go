...
func RegisterDefaults(scheme *runtime.Scheme) error {
	scheme.AddTypeDefaultingFunc(&TrainingJob{}, func(obj interface{}) { SetObjectDefaults_TrainingJob(obj.(*TrainingJob)) })
	scheme.AddTypeDefaultingFunc(&TrainingJobList{}, func(obj interface{}) { SetObjectDefaults_TrainingJobList(obj.(*TrainingJobList)) })
	return nil
}

func SetObjectDefaults_TrainingJob(in *TrainingJob) {
	SetDefaults_TrainingJob(in)
	if in.Spec.ReplicaSpecs != nil {
		for i := range in.Spec.ReplicaSpecs.Template.Spec.InitContainers {
			a := &in.Spec.ReplicaSpecs.Template.Spec.InitContainers[i]
			for j := range a.Ports {
				b := &a.Ports[j]
				if b.Protocol == "" {
					b.Protocol = "TCP"
		...
		for i := range in.Spec.ReplicaSpecs.Template.Spec.Containers {
			a := &in.Spec.ReplicaSpecs.Template.Spec.Containers[i]
			for j := range a.Ports {
				b := &a.Ports[j]
				if b.Protocol == "" {
					b.Protocol = "TCP"
		...
		for i := range in.Spec.ReplicaSpecs.Template.Spec.EphemeralContainers {
			a := &in.Spec.ReplicaSpecs.Template.Spec.EphemeralContainers[i]
...
func SetObjectDefaults_TrainingJobList(in *TrainingJobList) {
	for i := range in.Items {
		a := &in.Items[i]
		SetObjectDefaults_TrainingJob(a)
	}
}
