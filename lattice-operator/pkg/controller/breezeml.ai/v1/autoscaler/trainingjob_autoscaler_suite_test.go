...
import (
	...
	"github.com/breezeml/lattice-operator/pkg/config"
...
var _ = BeforeSuite(func() {
	...
	Expect(err).NotTo(HaveOccurred())
	err = breezemlv1.AddToScheme(scheme.Scheme)
	Expect(err).NotTo(HaveOccurred())

	// Set default config.
	config.Config.TrainingInitContainerImage = config.TrainingInitContainerImageDefault
	config.Config.TrainingInitContainerTemplateFile = config.TrainingInitContainerTemplateFileDefault
	...
	latticeConfig := latticeconfig.LatticeConfig{
		...
		JFrogSecret:     nil,
		ResourceUnit:    "node",
		DebugWorldSize:  &worldSize,
	}

	r := reconciler.NewReconciler(mgr, billingDaemon, &latticeConfig)
	autoscaler := NewReconciler(mgr, billingDaemon, &latticeConfig)
	Expect(r.SetupWithManager(mgr)).NotTo(HaveOccurred())
	Expect(autoscaler.SetupWithManager(mgr)).NotTo(HaveOccurred())
