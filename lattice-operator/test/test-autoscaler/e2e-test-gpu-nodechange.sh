...
# ========================= TEST 3: create a new node with another 5 gpus =========================
echo "Creating another node with 5 gpus..."
kubectl --context=kwok-fake-cluster apply -f test/test-autoscaler/kwok/node-gpus-2.yaml
sleep 10
...
# ========================= TEST 4: create the thied job with 3-5 nodes =========================
echo "Creating the second job with 2-4 pods..."
kubectl --context=kwok-fake-cluster apply -f test/test-autoscaler/jobs/trainingjob4_gpu.yaml

until [ $(kubectl --context=kwok-fake-cluster get trainingjob lattice-simple4 -n lattice --output=jsonpath='{@.status.stage}') = "Running" ]
do
    echo "Waiting for the job to run..."
    sleep 1
done
...
# Count the number of pods for the second job
check_pod_number "lattice-simple2" 3
if [ "$?" == 1 ]; then
    echo -e "\033[31mTest failed! Expected number of pods count is 3. \033[0m"
    passed=false
fi

# Count the number of pods for the third job
check_pod_number "lattice-simple4" 3
if [ "$?" == 1 ]; then
    echo -e "\033[31mTest failed! Expected number of pods count is 3. \033[0m"
    passed=false
fi
...
kubectl --context=kwok-fake-cluster delete -f test/test-autoscaler/kwok/node-gpus.yaml
sleep 10
# Count the number of pods for the first job
check_pod_number "lattice-simple1" 3
if [ "$?" == 1 ]; then
    echo -e "\033[31mTest failed! Expected number of pods count is 3. \033[0m"
    passed=false
fi
...
# Count the number of pods for the third job
check_pod_number "lattice-simple4" 0
if [ "$?" == 1 ]; then
...
kubectl --context=kwok-fake-cluster delete -f test/test-autoscaler/jobs/trainingjob4.yaml -n lattice
